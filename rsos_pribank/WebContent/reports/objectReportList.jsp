...
<html>
	...
	<script type="text/javascript">
			$(function(){
				$('#t_object').datagrid({
					...
					fit:true ,
					height:450 ,
					url:'objectReport!getObjectReportList.action',
					fitColumns:true ,  
					striped: true ,					//隔行变色特性 
					...
					columns:[[
						...
						{field:'clientId' ,title:'客户号' ,width:50 ,align:'center'},
						{field:'clientName' ,title:'客户名' ,width:50 ,align:'center'},
						{field:'originalAgentName' ,title:'原归属人' ,width:80 ,align:'center'},
						{field:'originalBranchBank' ,title:'原归属机构' ,width:80 ,align:'center'},
						{field:'newAgentName' ,title:'新归属人' ,width:80 ,align:'center'},
						{field:'newBranchBank' ,title:'新归属机构' ,width:80 ,align:'center'},
						{field:'objectApplyerName' ,title:'异议申请人' ,width:50 ,align:'center'},
						{field:'applyDepartmentName' ,title:'异议申请人所属机构' ,width:80 ,align:'center'},
						{field:'applyDate' ,title:'申请日期' ,width:100 ,align:'center'},
						{field:'endDate' ,title:'流程结束日期' ,width:100 ,align:'center'},
						{field:'fdevApplyerName' ,title:'深度申请人' ,width:80 ,align:'center'},
						{field:'fdevApplyDepartment' ,title:'深度申请人归属机构' ,width:80 ,align:'center'},
						{field:'result' ,title:'审批结果' ,width:50 ,align:'center'}	
					]] ,
					pagination: true , 
					pageSize: 10 ,
					pageList:[10,20,50] ,
					toolbar:[
						{
							text:'查看异议详情' ,
							iconCls:'icon-search' , 
							handler:viewObjectInfo
						},{
							text:'导出选中明细' ,
							iconCls:'icon-print' , 
							handler:function(){
								var arr =$('#t_object').datagrid('getSelections');
								if(arr.length <=0){
									...
									var url = 'objectReport!exportExcel.action?ids='+ids;
				...
				$('#exportComfirm').click(function(){
					...
					var url = 'objectReport!exportExcelMonthly.action?yearMonth='+yearMonth;
					window.location.href=url;
					$('#exportExcelMonthlyDialog').dialog('close');
				});
				
				$('#searchbtn').click(function(){
					$('#t_object').datagrid('load' ,serializeForm($('#mysearch')));
				});				
				
				$('#clearbtn').click(function(){
					$('#mysearch').form('clear');
					$('#t_object').datagrid('load' ,{});
				});			
			});
		function viewObjectInfo(){
			...
			if(arr.length <=0){
				...
				$('#viewObjectDialog').dialog('open').dialog("refresh","reports/viewObjectInfo.jsp?businessId='"+businessId+"'");
			}
		}
		//js方法：序列化表单 			
  ...
  <body style="overflow:hidden">
	<div id="lay" class="easyui-layout" style="width:100%;height:100%" >
		<div region="north" title="异议报表查询" style="height:100%;overflow:hidden" > 					
			<form id="mysearch" method="post">
				<table width="100%">
					...
					<tr>								
						...
						<td width="25%">申请人机构:<input name="department" value="" style="width:60%;height:22px;border:1px solid #A4BED4;"/></td>	
	...
	<div id="viewObjectDialog" title="查看异议审批信息" modal=false  draggable=true class="easyui-dialog" closed=true style="background-color:#E4F5EF;width:800px;height:520px;overflow:hidden" >
	</div>
  </body>
</html>