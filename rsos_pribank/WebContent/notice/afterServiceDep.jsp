...
<html>
  <head>
	...
	<script type="text/javascript">
			$(function(){
				var currentPage = 'afterServiceDep';
				...
				$('#t_noticeList').datagrid({
					...
					fit:true ,
					height:450 ,
					url:'fdevNoticeAction!findFdevNoticeList.action',
					fitColumns:true ,  
					striped: true ,					
					...
					columns:[[
						...
						{field:'userName',title:'查阅人',hidden:true}
					]] ,
					pagination: true , 
					pageSize: 10 ,
					pageList:[10,20,50],
			...
			function viewNotice(){
				...
				if(arr.length <=0){
					...
					$.ajax({
						type: 'post',
						url: 'fdevNoticeAction!viewNoticeAction.action',
						cache:false ,
						data:params,
						dataType:'json' ,
						success:function(result){
							if(result.noticeId!=null&&result.noticeId.toString().length>0){
								$("#object").val(result.userName);
  ...
  <body style="overflow:hidden">
	...
	<div id="viewNotice" title="查阅通知" modal=false  draggable=true class="easyui-dialog" closed=true style="width:600px;height:250px" >
