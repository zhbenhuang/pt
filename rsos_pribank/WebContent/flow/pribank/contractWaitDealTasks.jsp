...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		var status = "请输入条码";
		var businessId;
		var currentPage = 'contractWaitDealTasks';
		var flag ;
		$(function(){
			$('#status').combobox({
				editable:false,
			...
			$('#t_taskList').datagrid({
				...
				frozenColumns:[[
					...
					{field:'codeId' ,title:'合同条码' ,width:140},
				...
				toolbar:[
					...
					$('#showInfo').click(function(){
						...
						if(codeId==''){
							...
							$.ajax({
								type: 'post',
								url: 'contractProcess!showContractInfoAction.action',
								cache:false ,
								data:params,
								dataType:'json' ,
								success:function(result){
									if(result.redCode=="A000000"){
										...
										$("#codeId").val(result.contract.codeId);
										$("#remark").val(result.contract.remark);
									}else{
					...
					$('#btn1').click(function(){
						...
						if(codeId==""){
							...
							}else if(status.indexOf("待信托返回")>=0){
								LG.showWarn("提示消息","任务已提交信托,请等待信托返回!");
							}else if(status.indexOf("合同待领取")>=0){
								LG.showWarn("提示消息","信托已返回,等待支行领取!");
							...
							LG.showWarn("提示消息","产品编码未显示,请联系产品管理岗人员,确认系统中是否存在该产品!");
							 ...
							 if(status.indexOf("待审核")>=0){
								 ...
								 if(r){
									 $.ajax({
											type: 'post' ,
											url: 'contractProcess!passAction.action',
											cache:false,
								...
								LG.showWarn("提示消息","已提交信托,请等待信托返回!");
							 }else if(status.indexOf("合同待领取")>=0){
								LG.showWarn("提示消息","信托已返回,等待支行领取!");
							 }else if(status.indexOf("请输入条码")>=0){
								LG.showWarn("提示消息","请输入条码!");
							 ...
							 if(status.indexOf("待提交信托")>=0){
								$.messager.confirm('提示消息','确认提交信托?',function(r){
									if(r){
										$.ajax({
											type: 'post' ,
											url: 'contractProcess!submitXinTuoAction.action',
											cache:false,
											data:$('#myformtwo').serialize(),
											dataType:'json',
											success:function(result){
					...
					$('#xintuoback').click(function(){
						...
						if(codeId==""){
							...
							LG.showWarn("提示消息","产品编码未显示,请联系产品管理岗人员,确认系统中是否存在该产品!");
							 ...
							 if(status.indexOf("待信托返回")>=0){
								$.messager.confirm('提示消息','确认信托返回?',function(r){
									if(r){
										$.ajax({
											type: 'post' ,
											url: 'contractProcess!xinTuoBackAction.action',
											cache:false,
											data:$('#myformtwo').serialize() ,
											dataType:'json',
											success:function(result){
					...
					$('#getContract').click(function(){
						...
						if(codeId==""){
							...
							LG.showWarn("提示消息","产品编码未显示,请联系产品管理岗人员,确认系统中是否存在该产品!");
							 ...
							 if(status.indexOf("合同待领取")>=0){
								$.messager.confirm('提示消息','确认领取合同?',function(r){
									if(r){
										$.ajax({
											type: 'post' ,
											url: 'contractProcess!getContractAction.action',
											cache:false,
											data:$('#myformtwo').serialize() ,
											dataType:'json',
											success:function(result){
  ...
  <body style="overflow:hidden">
	<div id="lay" class="easyui-layout" style="width:100%;height:100%" >
				...
				<table width="100%">
					<tr>
	    				...
	    				<select id="status" class="" name="status" style="width:100%;">
	    					...
	    					<option value="待信托返回">待信托返回</option>
	    					<option value="合同待领取">合同待领取</option>
	    				</select>
	    				</td>	
		...
		<div id="mydialogOne" title="扫描条码" modal=false  draggable=true class="easyui-dialog" closed=true style="background-color:#E4F5EF;width:800px;height:540px">
    	<div id="lay" class="easyui-layout" style="width:100%;height:100%" >
    		<div region="west" split="true" style="background-color:#E4F5EF;width:160px;height:35%">
    			<table>
    				<tr>
    					<td>&nbsp;扫描条码:</td>
    				</tr>
    				...
    				<tr>
    					<td>
    						...
    						<a href="javascript:void(0)" id="closeBtn2" class="easyui-linkbutton">关闭</a>
    					</td>
    				</tr>   					 					    					    					    					    					    					    					    					
    			</table>
    		</div>
				...
				<table>
    				<tr>
    					<td colspan="2">
    						...
    						<a href="javascript:void(0)" id="xintuoback" class="easyui-linkbutton">信托返回</a>
    						<a href="javascript:void(0)" id="getContract" class="easyui-linkbutton">领取合同</a>
    						<a href="javascript:void(0)" id="closeBtn3" class="easyui-linkbutton">关闭</a>
    					</td>
    				</tr> 
    			</table>  					 					    					    					    					    					    					    					    					
    		</div>
    		<div region="center" style="overflow:auto;background-color:#E4F5EF;width:100%;height:100%">
	    		...
	    		<form id="myformtwo" action="" method="post">
	    			...
	    			<input type="hidden" id="business" name="businessId"/>
	    		 	<div class="title" align="center"><h4>理财产品合同流程</h4></div>
						<div align="center">
							<table width="90%" border="1" align="center" style="background-color:#E4F5EF;table-layout:fixed;">
								<tr>
		    						...
		    						<td width="30%"><input id="codeId" type="text" name="codeId" value="" readonly="readonly" style="background-color: #E1E6E9;width:86%;height:22px;border:1px solid #A4BED4;"/></td>
								</tr>
								<tr>
