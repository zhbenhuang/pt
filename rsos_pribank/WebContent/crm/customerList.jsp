...
<html>
  <head>
	...
	var actionMethod="saveCustomer";		//判断新增和修改方法 
	$(function(){
		...
		$('#t_customer').datagrid({
			...
			fit:true ,
			height:450 ,
			url:'queryCustomerList!queryCustomerList.action' ,
			fitColumns:true ,  
			striped: true ,
		...
		$('#confirmAdd').click(function(){
			if($('#newCustomerForm').form('validate')){				
				$.ajax({
					type: 'post' ,
		...
		$('#searchbtn').click(function(){
			$('#t_customer').datagrid('load' ,serializeForm($('#mysearch')));
		});
							
		$('#clearbtn').click(function(){
			$('#mysearch').form('clear');
			$('#t_customer').datagrid('load' ,{});
		});
	...
	function modifyCustomer(){
		actionMethod = 'modifyCustomer';
		var grid =$('#t_customer').datagrid('getSelections');
		if(grid.length != 1){
			...
			$('#newCustomerDialog').dialog('open'); //打开窗口
			$('#newCustomerForm').get(0).reset();   //清空表单数据 
			$('#newCustomerForm').form('load',{	   //调用load方法把所选中的数据load到表单中,非常方便
		...
		if(grid.length <=0){
			...
			$.messager.confirm('提示信息' , '确认删除?' , function(r){
				if(r){
					...
					$.post('deleteCustomer!deleteCustomer.action' , 
						{ids:ids} , 
						function(result){
							$.messager.alert('提示信息' , result.message,'info',function(){
								if(result.retCode=="A000000"){													
									...
									$('#t_customer').datagrid('reload');
									//2 清空idField   
									$('#t_customer').datagrid('unselectAll');	
								}else{
	...
	<div id="lay" class="easyui-layout" style="width: 100%;height:100%" >
		<div region="center" style="width: 100%;height:100%">
			<table id="t_customer" style="width: 100%;height:100%"></table>
		</div>
	</div>
	
	
  	
  	<div id="newCustomerDialog" title="新增客户信息" class="easyui-dialog" style="width:300px;height:500px;background-color:#E4F5EF;"  
        data-options="iconCls:'icon-save',modal:true,draggable:true,closed:true">
		<form id="newCustomerForm" action="" method="post"  align="center">
